 ...
 */
#ifndef WDTTIME
#define WDTPERIOD WATCHDOG_2S // 2 seconds default
#elif WDTTIME == 1
...
void virtualBootPartition(addr16_t mybuff, addr16_t address);
#endif
...
#if BIGBOOT
  // If first two adresses are not empty
  if ((eeprom_read(0x000) != 0xFF) && (eeprom_read(0x001) != 0xFF))
  {
    uint8_t i, j;
    static const uint16_t pagesize = 128;
    ...
    for (i = 0; i < 8; ++i) // 8 pages total
    {
      ...
      uint8_t *code;
      code = buff.bptr;
      address.word = (i * pagesize);

      for (j = 0; j < pagesize; ++j)  // 128 words at a time
      {     
        if (j % 64 == 0)     
          LED_PORT ^= _BV(LED);
      ...
      writebuffer('F', buff, address, (pagelen_t) pagesize);
    }
...
#if BIGBOOT
  uint16_t aaa;
  for (aaa = 0; aaa < 1024; aaa++)
     putch(eeprom_read(aaa));
#endif
